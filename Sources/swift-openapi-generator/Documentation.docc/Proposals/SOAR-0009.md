...
In OpenAPI, the operation could look like this (irrelevant parts were omitted, see the full OpenAPI document in [Appendix A][appendix-A]):
...
In addition to `MultipartBody`, we are proposing a few new public types (`MultipartPart` and `MultipartRawPart`) in the runtime library that are used in the Swift snippets below. The full proposed runtime library API diff can be found in [Appendix B][TODO], and the details of each type will be discussed in "Detailed design" section.
...
This section dives deeper into the details of the different cases we propose to support, and how to handle them.

#### Validation (TODO: move lower down)
...
Optionality of parts is not reflected as an `Type?` here, instead the absence of a part in the async sequence represents it being nil.
...
> Note: As a reminder, the full API of the types `MultipartPart`, `MultipartRawPart`, and `MultipartDynamicallyNamedPart` can be found in [Appendix B][TODO].

In this section, we will enumerate the different enum case types and under what circumstances they will be generated.

- **Scenario A**: Zero or more documented cases and `additionalProperties` are not set - a common default.
...
- **Scenario B**: Zero or more documented cases and `additionalProperties: true`.
    ...
    - Note that while similar to `undocumented`, the `other` case uses a different name to communicate the fact that the OpenAPI author deliberately enabled `additionalProperties` and thus any parts with unknown names are expected – so the `undocumented` name would not be appropriate.
...
- **Scenario C**: Zero or more documented cases and `additionalProperties: <SCHEMA>`.
    ...
    - Note that while similar to `MultipartPart`, `MultipartDynamicallyNamedPart` adds a read-write `name` property, because while the part name is statically known when `MultipartPart`, that's not the case when `MultipartDynamicallyNamedPart` is used, thus the extra property into which the part name is parsed is required.
...
`RandomizedMultipartBoundaryGenerator` uses a constant prefix and appends a random suffix made out of `0-9` digits, returning a different output every time.

By default, the updated `Configuration` uses the randomized boundary generator, but the adopter can switch to the constant one, or provide a completely custom implementation.

#### The Encoding OpenAPI field

In an example above, we saw how to use the `encoding` field in the OpenAPI document to:
1. explicitly specify the content type.
2. define custom headers for a part.
...
The generator follows these rules and once a serialization method is chosen, treats the payloads the same way as in regular single-body HTTP requests and responses.

Custom headers are also optional, so if the default content type is correctly inferred, and the adopter doesn't need any custom headers, the `encoding` object can be omitted in the OpenAPI document.

> Important: Note that the generated code depends on the pair of a `schema` + `encoding`, so in theory the generator would need to generate as many variants as there are unique combinations of `schema` + `encoding`. However, we choose not to do that, and instead will emit a diagnostic if a `schema` is associated with more than one `encoding` value, and always use the first pair encountered. We validated against almost 2000 open-source OpenAPI documents that not a single document used a `schema` with more than one `encoding`, so we are confident this will not limit any legitimate use cases.
...
The feature needs to be balanced against the cost of generating another variant of the code we will already generate with this proposal, and it's also important to not make it more appealing than the streaming variant, as then even adopters who would benefit from streaming might not use it, because they might see the buffered type first and not realize multipart streaming is even supported.
...
The first obvious alternative to adding type-safe support for multipart is not to do it. It has the advantage of preserving the streaming nature, and doesn't force buffering on users.

However, better support for multipart has been the top user request in recent months, so it seemed clear that the status quo is not sufficient. On the wire, multipart is not trivial to serialize and parse correctly, so asking every adopter to reimplement it seemed suboptimal. 
...
Even with type-safe generated parts, as proposed, we could avoid doing runtime validation of the part semantics coming from the OpenAPI document, such as that a required part did actually arrive before the multipart body sequence was completed, and that only parts described by an array schema are allowed to appear more than once.

While that would simplify implementation by a little bit, it would again weaken the trust the adopters can have in the type-safe code truly verifying as much information as possible from the OpenAPI document.

The verification happens in a private async sequence that's inserted into the middle of the serialization/parsing chain, so is mostly implemented in the runtime library, not really affecting the generator implementation complexity very much.
...
extension MultipartBoundaryGenerator where Self == OpenAPIRuntime.RandomizedMultipartBoundaryGenerator {

    /// A generator that produces a random boundary every time.
    public static var randomized: OpenAPIRuntime.RandomizedMultipartBoundaryGenerator { get }
}
...
public struct RandomizedMultipartBoundaryGenerator : OpenAPIRuntime.MultipartBoundaryGenerator {
...
/* public struct Configuration : Sendable { */
    ...
    public init(dateTranscoder: OpenAPIRuntime.DateTranscoder = .iso8601, multipartBoundaryGenerator: OpenAPIRuntime.MultipartBoundaryGenerator = .randomized)
