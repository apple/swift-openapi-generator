...
struct ContentType: Hashable {
    ...
    init(_ rawValue: String) {
        ...
        let rawTypeAndSubtype = rawValue.split(separator: ";")[0].trimmingCharacters(in: .whitespaces)
    ...
    var lowercasedTypeAndSubtypeWithEscape: String { "\(lowercasedType)\\/\(lowercasedSubtype)" }

    /// The header value used when sending a content-type header.
    var headerValueForSending: String {
        guard case .json = category else { return lowercasedTypeAndSubtype }
...
extension OpenAPI.ContentType {
    ...
    /// A Boolean value that indicates whether the content type
    /// is a type of JSON.
    var isJSON: Bool { asGeneratorContentType.isJSON }

    /// A Boolean value that indicates whether the content type
    /// is a type of plain text.
    var isText: Bool { asGeneratorContentType.isText }

    /// A Boolean value that indicates whether the content type
    /// is just binary data.
    var isBinary: Bool { asGeneratorContentType.isBinary }
    ...
    var asGeneratorContentType: ContentType { ContentType(typeAndSubtype) }
}
