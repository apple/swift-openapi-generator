...
@Suite("Operation Description Tests")
struct Test_OperationDescription {
    ...
    private func makeOperationDescription(_ pathItem: OpenAPI.PathItem) -> OperationDescription? {
        guard let endpoint = pathItem.endpoints.first else {
            Issue.record("Unable to retrieve the path item first endpoint.")
            return nil
        }
    ...
    @Test("Duplicate parameters retain only operation parameters")
    func testAllParameters_duplicates_retainOnlyOperationParameters() throws {
        ...
        #expect(allParameters == [operationLevelParameter])
    }
    
    @Test("All parameters keep duplicates from different locations")
    func testAllParameters_duplicates_keepsDuplicatesFromDifferentLocation() throws {
        ...
        #expect(allParameters == [pathLevelParameter, operationLevelParameter])
    }

    @Test("All parameters are returned in expected order")
    func testAllParameters_duplicates_ordering() throws {
        ...
        #expect(allParameters == [pathLevelParameter, duplicatedParameter, operationLevelParameter])
    }
    
    @Test("Response outcomes match expected values without default response")
    ...
    @Test("Default response is the last outcome")
    ...
    @Test("Default response is ordered first in response outcomes")
