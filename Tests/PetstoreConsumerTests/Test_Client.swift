...
final class Test_Client: XCTestCase {
    ...
    func testListPets_200() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "listPets")
            ...
            XCTAssertEqual(
                ...
                [.init(name: "My-Request-UUID", value: "abcd-1234"), .init(name: "accept", value: "application/json")]
            )
        ...
        let response = try await client.listPets(
            .init(
                ...
                headers: .init(My_Request_UUID: "abcd-1234")
            )
        )
    ...
    func testListPets_default() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "listPets")
            ...
            XCTAssertEqual(baseURL.absoluteString, "/api")
            XCTAssertEqual(request.method, .get)
            XCTAssertEqual(request.headerFields, [.init(name: "accept", value: "application/json")])
            XCTAssertNil(request.body)
            return .init(
                statusCode: 400,
        ...
        switch value.body {
        case .json(let error):
            XCTAssertEqual(error, .init(code: 1, me_sage: "Oh no!", userData: try .init(unvalidatedValue: ["one": 1])))
        }
    }

    func testCreatePet_201() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "createPet")
            XCTAssertEqual(request.path, "/pets")
        ...
        let response = try await client.createPet(
            .init(headers: .init(X_Extra_Arguments: .init(code: 1)), body: .json(.init(name: "Fluffz")))
        )
    ...
    func testUpdatePet_204_withBody() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "updatePet")
            ...
            return .init(statusCode: 204, headerFields: [.init(name: "content-type", value: "application/json")])
        }
    ...
    func testGetStats_200_json() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "getStats")
    ...
    func testGetStats_200_unexpectedContentType() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "getStats")
            ...
            return .init(
                statusCode: 200,
                headers: [.init(name: "content-type", value: "foo/bar")],
                encodedBody: #"""
                    count_is_1
                    """#
            )
    ...
    func testPostStats_202_json() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "postStats")
            XCTAssertEqual(request.path, "/pets/stats")
            XCTAssertNil(request.query)
            XCTAssertEqual(baseURL.absoluteString, "/api")
            XCTAssertEqual(request.method, .post)
    ...
    @available(*, deprecated) func testProbe_204() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "probe")
    ...
    @available(*, deprecated) func testProbe_undocumented() async throws {
        transport = .init { request, baseURL, operationID in .init(statusCode: 503) }
        let response = try await client.probe(.init())
    ...
    func testUploadAvatarForPet_200() async throws {
        transport = .init { request, baseURL, operationID in XCTAssertEqual(operationID, "uploadAvatarForPet")
            ...
            return .init(
                statusCode: 200,
                headers: [.init(name: "content-type", value: "application/octet-stream")],
                encodedBody: Data.efghString
            )
        }
        let response = try await client.uploadAvatarForPet(.init(path: .init(petId: 1), body: .binary(.abcd)))
        guard case let .ok(value) = response else {
            XCTFail("Unexpected response: \(response)")
            return
        }
        switch value.body {
        case .binary(let binary): XCTAssertEqualStringifiedData(binary, Data.efghString)
        }
    }
    ...
    func testUploadAvatarForPet_500() async throws {
        ...
        switch value.body {
        case .text(let text): XCTAssertEqual(text, Data.efghString)
        }
    }
}
