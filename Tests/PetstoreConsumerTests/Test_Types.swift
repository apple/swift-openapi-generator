...
final class Test_Types: XCTestCase {
    ...
    func testStructCodingKeys() throws {
        let cases: [(Components.Schemas._Error.CodingKeys, String)] = [(.code, "code"), (.me_sage, "me$sage")]
        for (value, rawValue) in cases { XCTAssertEqual(value.rawValue, rawValue) }
    }

    func testEnumCoding() throws {
        let cases: [(Components.Schemas.PetKind, String)] = [(.cat, "cat"), (._nake, "$nake")]
        for (value, rawValue) in cases { XCTAssertEqual(value.rawValue, rawValue) }
    }
    ...
    func testOneOfAny_roundtrip() throws {
        ...
        try _testRoundtrip(Components.Schemas.OneOfAny.undocumented(true))
    }
    ...
    func testOneOfWithDiscriminator_roundtrip() throws {
        ...
        try _testRoundtrip(
            Components.Schemas.OneOfObjectsWithDiscriminator.undocumented(.init(unvalidatedValue: ["kind": "nope"]))
        )
    }
